...
#drivetrain
@export var Powered_Wheels = ["fl","fr"]

@export var FinalDriveRatio = 4.250
@export var GearRatios = [ 3.250, 1.894, 1.259, 0.937, 0.771 ]
...
func aero():
	...
	var veloc = global_transform.basis.orthonormalized().inverse() * linear_velocity
	
	var torq = global_transform.basis.orthonormalized().inverse() * Vector3(1,0,0)
...
func _physics_process(delta):
	...
	if Use_Global_Control_Settings:
		...
		SteerSensitivity = VitaVehicleSimulation.SteerSensitivity
	...
	velocity = global_transform.basis.orthonormalized().inverse() * linear_velocity
	rvelocity = global_transform.basis.orthonormalized().inverse() * angular_velocity
	...
	gforce = global_transform.basis.orthonormalized().inverse() * gforce
