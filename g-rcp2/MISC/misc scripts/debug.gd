...
func _process(delta):
	get_node("../sun").shadow_enabled = misc_graphics_settings.shadows
	...
	if not car == "":
		...
		$power_graph/redline.rect_position.x = (get_node(car).RPMLimit/$power_graph.Generation_Range)*$power_graph.rect_size.x -1.0
